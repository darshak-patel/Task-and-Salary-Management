...
<?php
	...
	if($Name=='SelectTeamLeader')
	{
			...
			$Query="INSERT INTO `tbl_targetassignteamleader`(`EmployeeID`, `ProductID`, `TargetA`, `AssignEmp`) VALUES ('$EmpName','$Product','$Target','$Emp')";
...
<body onLoad="JSconfirm()">
    <script type="text/javascript">
    	function JSconfirm(){
    	swal({
      			title: "Our Target Sent To Emplyoee Is Succesfully!",
      			icon: "success",
    		})
    	.then((willDelete) => {
      	if(willDelete) 
		{
         	window.location = "AssignTargetTeamLeader.php";     
        }  
        else 
		{
        window.location = "AssignTargetTeamLeader.php";     
